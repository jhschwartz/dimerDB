import numpy as np
from scipy import AgglomerativeClustering
import rep_helpers
import name_pdb
...
def choose_rep(dimers, all_dimers, dist_mat, lib_path, resolu_path):
    ...
    :param dimers: list of dimers, each a tuple of two strings that are
                    dimer names from name_pdb, of which we wish to
    ...
    #### CRITERION 1 ####
    coverages = [rep_helpers._dimer_coverage(dimer) for dimer in dimers]
    ...
    for dimer in choices:
        res = _get_dimer_avg_resolu(dimer)
        if res != -1.0: # is xray
    ...
    #### CRITERION 3 ####
    choices = rep_helpers._dimers_of_lowest_distance_to_others(  group=choices,
                                                                 dist_mat=dist_mat,
    ...
    if len(xray_dimers) > 0:  # if we are working with xray dimers
        resolus = []
        for dimer in choices:
            resolus.append(_get_dimer_avg_resolu(dimer))
        min_res = min(resolus)
        best_xray = []
