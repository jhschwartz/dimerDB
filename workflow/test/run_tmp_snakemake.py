...
@contextmanager
def run_tmp_snakemake(snakefile, config, snakemake_exe, scripts, bin, rule=None, lib=None, intermediates=None):
    init_dir = os.getcwd()
    try:
        td = tempfile.mkdtemp()
        ...
        if intermediates:
            shutil.copytree(intermediates, os.path.join(td, 'intermediates'))
        ...
        if rule:
            rule_info = f'-R {rule}'
        subprocess.run(f'{snakemake_exe} -s {tmp_snakefile} -c32 {rule_info} --printshellcmds', shell=True, check=True)
        yield tmp_snakefile
    finally:
        shutil.rmtree(td)
        os.chdir(init_dir)
